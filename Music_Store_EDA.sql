...
select employee_id,first_name,last_name,levels from employee order by levels limit 1;
...
select cust.first_name,cust.last_name,sum(invoice.total) as total
from customer cust join invoice on cust.customer_id = invoice.customer_id
group by cust.customer_id order by total desc limit 1;
...
JOIN best_sell_artist bsa ON bsa.artist_id = alb.artist_id
GROUP BY 1,2,3,4
ORDER BY 5 DESC;
